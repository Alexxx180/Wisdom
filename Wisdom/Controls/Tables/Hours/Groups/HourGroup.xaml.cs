...
namespace Wisdom.Controls.Tables.Hours.Groups
{
    ...
    /// </summary>
    public partial class HourGroup : UserControl, INotifyPropertyChanged, IRawData<List<Hour>>, IExtendableItems
    {
        ...
        public List<Hour> Raw()
        {
            return Hours.GetRaw();
        }

        public void SetElement(List<Hour> values)
        {
            for (ushort i = 0; i < values.Count; i++)
            {
                HourElement hour = new HourElement();
                hour.SetElement(values[i]);
                Hours.Add(hour);
            }
        }
        #endregion
