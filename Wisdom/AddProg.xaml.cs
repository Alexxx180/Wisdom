...
namespace Wisdom
{
    ...
    public partial class AddProg : Window, INotifyPropertyChanged
    {
        ...
        {
            InitializeComponent();
            ViewModel = new DisciplineProgramViewModel(viewModel);
            SetUp();
        }
        ...
        {
            System.Diagnostics.Trace.WriteLine(ViewModel.HourGroups[0].Name);
            System.Diagnostics.Trace.WriteLine(ViewModel.HourGroups[0].Count);
            return;
            if (Page5.WasChanged())
                SavePreferences();
